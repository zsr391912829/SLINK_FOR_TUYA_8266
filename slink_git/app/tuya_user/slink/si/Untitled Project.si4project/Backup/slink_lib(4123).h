...
/*以下根据不同的云端进行定制*/
//extern void slink_link_Init(char *slink_prouctKey,void *device_cb,);
extern char slink_link_send_str_to_tuya(char *str,int len);

typedef void (*slink_link_tuya_recv_cb)();
extern void slink_link_register_tuya_recv_cb(char *str,int rec_len);
